...
# Contoh untuk mengisi kolom slug di dalam tabel obat
openDb()

cursor.execute("SELECT kode_obat, nama_obat FROM obat")
products = cursor.fetchall()

for product in products:
    kode_obat = product[0]
    nama_obat = product[1]
    slug = slugify(nama_obat)
    
    cursor.execute("UPDATE obat SET slug = %s WHERE kode_obat = %s", (slug, kode_obat))
    db.commit()
...
@app.route('/about')
def about():
    return render_template('rst.html')
